...
- **Server-side AFAS bool filters are unreliable.** Always pull-and-filter in PHP (see AGENTS.md of `afas-connector-tools`).
- **Per-language naming templates are the source of truth, not the current AFAS data.** AFAS contains drift (typos, casing, missing prefixes — see `PLAN-AFAS.md` §9.1). The tool normalises *toward* the templates, never the reverse. Templates only change with explicit user approval.
...
## Files Codex should keep up to date

- `PLAN.md` — living design document.
- `TODO.md` — phased checklist driven by `PLAN.md`.
- `AGENTS.md` — this file. Update only when the user changes the working agreement.
